# -*- coding: utf-8 -*-
"""
Created on Fri May 04 13:41:42 2018
...
def T_Rocio(y,P):
    ...
    Psatj = P*sum((y*PHI/gamma)*(Psat[0]/Psat)) 
    T = optimize.fsolve(lambda T:FO_Tsat(T,P,0),Tamb)[0]
    Psat = [PsatAntoine_6(i,T) for i in range(0,numero_s)]
    PHI = numpy.array([Phi_6(y,T,P)[i]/Phi_6(y,T,Psat[i])[i] for i in range(0,numero_s)])
    ...
    while ErrorT >= ErrorPermitidoT:
        T_E0 = T
        Psat = [PsatAntoine_6(i,T_E0) for i in range(0,numero_s)]
        PHI = numpy.array([Phi_6(y,T_E0,P)[i]/Phi_6(y,T_E0,Psat[i])[i] for i in range(0,numero_s)])
        ErrorGamma = numpy.ones(numero_s)
        ErrorPermitidoGamma = numpy.array([1.0E-5 for i in range(0,numero_s)])   
        while (ErrorGamma>=ErrorPermitidoGamma).all():
            gamma_E0 = gamma
            x = (y*PHI*P)/(gamma_E0*Psat)
            x = x/sum(x)
            gamma = Gamma_6(x,T_E0)
            ErrorGamma = abs(gamma_E0 - gamma)
        Psatj = P*sum((y*PHI/gamma)*(Psat[0]/Psat))
        def FO_T_E1(T,i):
            return Psatj - PsatAntoine_6(i,T)
...
#Grafica GRAFICA DE T-ROCIO
plt.plot(x_graf,T_graf, color='blue', linestyle='-', marker='', label='Linea de T-rocio')
plt.plot(y_graf,T_graf, color='green', linestyle='-', marker='', label='Linea de  T- burbuja')
plt.legend(loc=0)
plt.title('DIAGRAMA DE TEMPERATUA ROCIO ', color='k', size='20') 
